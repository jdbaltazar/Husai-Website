...
$result = mysql_query("SELECT * FROM service WHERE Discounted=True ORDER BY Service_Name;");
mysql_close($con);
?>
            ...
            <table width="560" border="2" cellspacing="15" cellpadding="0" id = "search-result">
				...
				while($row = mysql_fetch_array($result)){
					...
					if($row["File_Path"]!=null)
					 $image = $row["File_Path"];
					echo '<td > <div> <img src = '.$image.'></div> <p><span style="font-weight:bold;">'.$row["Service_Name"].'</span> <br/> <span style="font-style:italic;">P'.$row["Charge"].'('.$row["Discount"].'% off)<span></p> </td>';
