...
$result = mysql_query("SELECT * FROM product WHERE Discounted=True AND Status='Available' ORDER BY Product_Name;");
mysql_close($con);
?>
		...
		<div id="content-left">
			<table id="page-title">
				<tr>
					...
					<td>Products &raquo; Special/Discounted Products</td>
				</tr>
			</table>
			...
			<table width="530" border="2" cellspacing="15" cellpadding="0"
				...
				while($row = mysql_fetch_array($result)){
					...
					echo '<td > <div> <a href="../../upload/products/full_view/'.$image.'" rel="prettyPhoto" title="'.$row["Description"].'"><img src = "../../upload/products/thumbnail/'.$image.'" alt="'.$row["Product_Name"].'"></a></div> <p><span style="font-weight:bold;">'.$row["Product_Name"].'</span> <br/> <span style="font-style:italic;">P'.$row["Price"].'('.$row["Discount"].'% off)'.'<span></p> </td>';
